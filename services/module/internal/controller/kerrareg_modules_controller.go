...
func (r *KerraregReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	moduleVersionRefs := make(map[string]modulev1alpha1.ModuleVersionRef)
	for _, version := range module.Spec.Versions {
		...
		if err != nil {
			...
			if err != nil {
				r.Log.Error(err, "unable to generate UUID",
					"moduleVersion", version.Version,
					"module", module.Name,
				)
				return ctrl.Result{}, err
			...
			moduleVersionFileName := fmt.Sprintf("%s.%s", moduleVersionFileUUID, module.Spec.FileFormat)
			moduleVersion := &versionv1alpha1.ModuleVersion{
				ObjectMeta: v1.ObjectMeta{
					Name:      object.Name,
					Namespace: object.Namespace,
				},
				Spec: versionv1alpha1.ModuleVersionSpec{
					FileName: &moduleVersionFileName,
					Version:  version.Version,
					ModuleConfig: versionv1alpha1.ModuleConfig{
						Name: module.Name,
					},
				},
			}

			if module.Spec.GithubClientConfig.UseAuthenticatedClient {
				moduleVersion.Spec.ModuleConfig.GithubClientConfig.UseAuthenticatedClient = true
			}

			if module.Spec.StorageConfig.S3 != nil {
				moduleVersion.Spec.ModuleConfig.StorageConfig = versionv1alpha1.StorageConfig{
		...
		if module.Spec.GithubClientConfig.UseAuthenticatedClient {
			moduleVersion.Spec.ModuleConfig.GithubClientConfig.UseAuthenticatedClient = module.Spec.GithubClientConfig.UseAuthenticatedClient
		}

		if module.Spec.StorageConfig.S3 != nil {
			moduleVersion.Spec.ModuleConfig.StorageConfig.S3 = &versionv1alpha1.AmazonS3Config{
				Bucket: module.Spec.StorageConfig.S3.Bucket,
				Region: module.Spec.StorageConfig.S3.Region,
			}
			...
			moduleVersionRef := modulev1alpha1.ModuleVersionRef{
				Name:     moduleVersionName,
...
func (r *KerraregReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Owns(&versionv1alpha1.ModuleVersion{}).
