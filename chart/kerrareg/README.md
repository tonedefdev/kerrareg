...
A Helm chart for deploying Kerrareg, a self-hosted OpenTofu/Terraform Module and Provider Registry, on Kubernetes.

## Overview

This chart deploys five Kerrareg services:
...
All images are pulled from `ghcr.io/tonedefdev/kerrareg/` and default to the tag set in `global.image.tag`.

## Prerequisites

- Kubernetes 1.25+
- Helm 3.0+
- Kerrareg CRDs installed (see below)
...
To remove CRDs (this deletes all Kerrareg custom resources):

```bash
kubectl delete -f chart/kerrareg/crds/
```

## Quick Start

```bash
# Minimal install — uses kerrareg-system namespace by default
...
|-----------|---------|-------------|
| `global.namespace` | `kerrareg-system` | Kubernetes namespace for all resources |
...
| `version.image.repository` | `ghcr.io/tonedefdev/kerrareg/version-controller` | Image repository |
...
| `module.image.repository` | `ghcr.io/tonedefdev/kerrareg/module-controller` | Image repository |
...
| `depot.image.repository` | `ghcr.io/tonedefdev/kerrareg/depot-controller` | Image repository |
...
| `provider.image.repository` | `ghcr.io/tonedefdev/kerrareg/provider-controller` | Image repository |
...
| `server.image.repository` | `ghcr.io/tonedefdev/kerrareg/server` | Image repository |
...
When TLS is enabled the chart mounts a `Secret` named `kerrareg-tls` as a volume at the directory containing `certPath`. Create the secret before installing:
...
| `server.ingress.hosts` | `[{host: kerrareg.defdev.io, paths: [{path: /, pathType: Prefix}]}]` | Host/path routing rules |
...
| `server.ingress.istio.hosts` | `[kerrareg.defdev.io]` | Hosts for the VirtualService |
...
helm install kerrareg ./chart/kerrareg \
  ...
  --set server.gpg.secretName=kerrareg-gpg
```
...
helm install kerrareg ./chart/kerrareg \
  ...
  --set "server.ingress.tls[0].secretName=kerrareg-tls" \
  --set "server.ingress.tls[0].hosts[0]=kerrareg.example.com"
```
...
helm install kerrareg ./chart/kerrareg \
  ...
  --set "server.ingress.istio.hosts[0]=kerrareg.example.com" \
...
helm install kerrareg ./chart/kerrareg \
  ...
  --set storage.filesystem.hostPath=/mnt/kerrareg-modules
```
...
```bash
helm uninstall kerrareg -n kerrareg-system
```
